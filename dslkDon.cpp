...
/*
- Hóa đơn : Mã hóa đơn, Tên khách hàng, ngày tháng năm sinh, địa chỉ, số điện thoại, loại phòng, hạng phòng,so ngay thue
 ngày bắt đầu thuê,thành tiền thuê phòng, ngày lập hóa đơn, số lượng dịch vụ, tổng tiền phải thanh toán
- Bảng dịch vụ đã sử dụng: Mã dịch vụ, tên dịch vụ, số lượng, đơn giá, thành tiền

sldv nhap bao nhieu thi co bay nhieu dv
*/
struct dichVu{
   int maDV;
   char tenDV[50];
...
void chenDau(listHD &Q, NodeHD *e){
   if (Q.Head == NULL){
      Q.Head = e;
      Q.Tail = Q.Head;
   } else {
...
void chenCuoi(listHD &Q,NodeHD *e){
  if (Q.Head==NULL){
    Q.Head = e;
    Q.Tail =Q.Head;
  } else {
...
void themHoaDonVTBatKi(listHD &Q){
    int vt;
    int count=0;
    for(NodeHD *p = Q.Head;p!=NULL;p=p->next){
        count++;
    }
    hoaDon x;
    nhapHD(x);
    ...
    if (Q.Head ==NULL || vt ==1){
        ...
        for (NodeHD *k = Q.Head;k!=NULL;k=k->next){
            dem++;
            if (dem == vt){
...
void xoaHDTheoMa(listHD &Q){
 if (Q.Head!=NULL){
     ...
     cout<<"\nNhap ma hoa don muon xoa: ";cin>>ma;
          if (Q.Head->data.maHD==ma){                        //xoa dau
             NodeHD *p = Q.Head;
             Q.Head=Q.Head->next;
             delete p;
             if (Q.Head==NULL) Q.Tail=NULL;
          }
          else if (Q.Tail->data.maHD==ma){                 //xoa cuoi
                NodeHD *truoc ;
              for(NodeHD *k =Q.Head;k!=NULL;k=k->next){
                 if (k==Q.Tail){
                    Q.Tail = truoc;
   ...
   cout <<"Xoa thanh cong. Hay xuat lai danh sach de kiem tra!";
 } else {
   cout <<"Danh sach dang khong co hoa don nao!";
 }
}
